...
async function main() {
  ...
  console.log(
    "UniswapFlashloaner contract deployed to:",
    uniFlashloaner.address
  );

  const uniswapV2LibraryFactory = await ethers.getContractFactory(
    "UniswapV2Library"
  );
  const uniswapV2Library = await uniswapV2LibraryFactory.deploy();
  console.log(
    "UniswapV2Library contract deployed to:",
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        uniswapFlashloanerAddress: uniFlashloaner.address,
        uniswapV2LibraryAddress: uniswapV2Library.address,
      },
      undefined,
      2
    )
